...
// Helper Function to get time diff in seconds
...
// Optimal Uniform Init for Weights
...
// Modify softmax to work with batch inputs
...
// Modified Forward Function to handle Timing
...
// Modify Cross Entropy Loss to handle Batches
...
void update_weights_timed(NeuralNetwork *nn, TimingStats *stats) {
    ...
    for (int i = 0; i < HIDDEN_SIZE * INPUT_SIZE; i++) {
        nn->weights1[i] -= LR * nn->grad_weights1[i];
    }
    for (int i = 0; i < OUTPUT_SIZE * HIDDEN_SIZE; i++) {
        nn->weights2[i] -= LR * nn->grad_weights2[i];
    }
    for (int i = 0; i < HIDDEN_SIZE; i++) {
        nn->bias1[i] -= LR * nn->grad_bias1[i];
    }
    for (int i = 0; i < OUTPUT_SIZE; i++) {
        nn->bias2[i] -= LR * nn->grad_bias2[i];
    }
    clock_gettime(CLOCK_MONOTONIC, &end);
